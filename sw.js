const CACHE_NAME = "ejimpitan-v4-5-0-clean-core";
...
self.addEventListener("fetch", event => {
  ...
  event.respondWith(caches.match(event.request).then(cached => cached || fetch(event.request).then(r => {const c=r.clone(); caches.open(CACHE_NAME).then(x=>x.put(event.request,c)); return r;})));
});
